...
/**
 * Перерисовка — раз в кадр, а не на каждое изменение. События вкладок идут
 * пачками (загрузка страницы, счётчик блокировок, звук, заголовок фоновой
 * вкладки), и каждое перерисовывало строку вкладок, адресную строку и панели
 * целиком. Кадр собирает их в одну перерисовку перед отрисовкой экрана, так
 * что на глаз ничего не опаздывает. Само состояние меняется сразу — позже
 * приходят только подписчики. Свёрнутое окно кадров не рисует: тогда
 * перерисовку (а с ней и запись сессии) доводит таймер.
 */
let pending = false;
...
/** Перерисовать сейчас — тому, кто сразу после изменения читает DOM. */
